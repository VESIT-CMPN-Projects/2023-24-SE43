...
import user from '../assets/imges/user.jpg'
    ...
    return (
        <div key={job._id} className='w-full cursor-pointer  transition-all duration-1000  md:w-5/12 m-4 border hover:shadow-xl rounded px-4 md:flex md:flex-wrap'>
            <div className='mb-4 flex  items-center justify-center py-2 '>
                <Image width={70} height={70} className="flex rounded-full " src={user} alt="no image" />
                <div className='flex flex-col mx-2 px-2'>
                    {/* <h1 className='text-xl md:text-2xl font-semibold'>{job?.user}</h1> */}
            ...
            <div className='mb-4 flex   items-start justify-center py-2 flex-col'>
                <div className='flex  px-2 py-2 items-center justify-center '>
                    <BsDot className='text-4xl font-extrabold text-info-600' />
            ...
            <div className='mb-4 flex flex-col md:flex-wrap md:flex-row w-full justify-between  items-center '>
                ...
                <div className='mb-4 flex  items-start justify-center py-2 flex-col'>
                    <div className='flex px-6 rounded-2xl py-1 items-center justify-center bg-info-200 text-info-900  '>
                        <p>{job?.title} </p>
                    </div>
                </div>
                {
                    posted ? (
                        <button onClick={() => router.push(`/frontend/detailPostedJob/${job?._id}`)} className='my-2 py-2 px-4  border border-info-600   rounded flex items-center justify-center transition-all duration-700 hover:bg-info-600 hover:text-white text-info-600 font-semibold'>View Applications <AiOutlineArrowRight className='mx-2 text-xl' /></button>
                    ) : (

                        <button onClick={() => router.push(`/frontend/jobDetails/${job?._id}`)} className='my-2 py-2 px-4  border border-info-600   rounded flex items-center justify-center transition-all duration-700 hover:bg-info-600 hover:text-white text-info-600 font-semibold'>View Detail <AiOutlineArrowRight className='mx-2 text-xl' /></button>
                    )
                }
            </div>
        </div>
