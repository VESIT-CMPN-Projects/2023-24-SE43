...
export default function SavedJobDataTable() {
    ...
    const columns = [
        ...
        {
            name: '',
            cell: row => <button onClick={() => router.push(`/frontend/jobDetails/${row?.job?._id}`)} className='md:px-2 md:py-2 px-1 py-1 text-xs texinfo hover:text-white my-2 hover:binfo border bordeinfo   rounded transition-all duration-700  '>view Detail</button>,
        },
    ];
    ...
    return (
        <>
                        ...
                        <DataTable
                            ...
                            persistTableHead
                            subHeaderComponent={
                                <input className='w-60  py-2 px-2  outline-none  border-b-2 border-info-600' type={"search"}
                                    value={search}
